...
import { usernameAtom, usernameT } from "@/components/atoms/register";
...
const Register_2 = ({ setPage, user }: p) => {
  ...
  let isTerm = useRef(false);
   let userName = useRef<undefined | string>('Cryptomarkings');
  let {referral} = useParams();
  let [loading,setLoading] = useState(false);
  let code = useRecoilValue<usernameT>(usernameAtom)
  ...
  // let  data = referral[0];
  // let  data:any = code;
  console.log('you came from url: ',document.referrer)
  let  data:any = referral;
  if(data?.username?.length !==0) {
    userName.current = (data);
  }
  // console.log('refferral code is: ',referral);
  setLoading(true);
    ...
    <>
      {
        ...
        initialValues={{
          ...
          password: "",
          confirmp: "",
          referral_code:  userName.current, 
          terms: "",
          client_id: "VILA_BOT",
          position: "LEFT",
        }}
